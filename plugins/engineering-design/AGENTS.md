...
四个阶段，外加 `docs-init` 目录初始化器。产出统一落入本仓的 `docs/engineering/`
五段式目录，写作时的编号、状态与语言纪律全部由 `_shared/` 一处约束。
...
├── README.md
└── skills/
    ├── _shared/              # 四技能共享约束（单一事实源）
    │   ├── conventions.md    #   五阶段目录 + NNNN 全局编号命名
...
| 任务 | 位置 | 说明 |
| --- | --- | --- |
| 改编号或命名规则 | `skills/_shared/conventions.md` | 四技能同时生效 |
...
- **共享内容只进 `_shared/`**：编号、状态机、词表、语言纪律四技能共用一份事实源，
  复制进各技能必然版本漂移；下划线前缀保证它不会被当作技能发现。
...
- **样例共用一条主题线**：四技能的 `examples/sample.md` 以「构建产物入库」为
  同一主题连成 draft→research→rfc→adr 完整流转演示，改动需保持编号衔接连贯。
...
2. 编号冲突 → 编号取 `docs/engineering/` 全部子目录的全局最大值 +1，
   规则见 `_shared/conventions.md`；
