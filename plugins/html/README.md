...
- **纯画布独立导出（Diagram Canvas Only）**：
  - 复制图片到剪贴板、导出 2x PNG、导出原生矢量 SVG 与保真 PDF **100% 仅针对架构图画布内容进行**，生成纯净无外部界面杂质的工程蓝图；
  - 导出时视口与用户交互（Pan & Zoom）自动解耦，确保 SVG/PNG/PDF 始终导出规整完整的 100% 原图。
- **卡片内容四周内边距一致（对称居中）**：
  - 节点内部图元与文字整体居中对齐，左右留白与上下留白对称平衡，杜绝文字挤在左侧、右侧空荡的失衡排版。
