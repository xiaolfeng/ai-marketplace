...
- **Hero 模式 Header**：`2.25rem / 800` 超粗主标题，顶部带有 `PRODUCTION BLUEPRINT · ENTERPRISE` 生产级徽章；
- **画板自适应与自由平移缩放**：默认自适应撑满容器视窗呈现全景，支持鼠标拖拽平移、滚轮焦点缩放、右下角悬浮控制栏操作与双击还原；
- **纯画布导出流水线**：复制图片、导出 PNG、导出 SVG 与 PDF **100% 仅针对架构图画布（Diagram Canvas Only）进行**，生成标准的 2x 视网膜图像与单页比例 PDF，完全排除外部页面 Header、工具栏与图例；
- **卡片内容四周内边距一致（对称居中）**：所有节点内部图元（Lucide 图标 + 文本块）按照 $Padding_H = (W_{card} - W_{content}) / 2$ 精准居中对齐，左右和上下内边距对称一致，杜绝偏向一侧；
